...
contract OctoplaceMarket is ReentrancyGuard {
    ...
    function setTier(
        ...
        require(
            marketFeeMultiplierSale <= 100 &&
                creatorFeeMultiplierSale <= 100 &&
                marketFeeMultiplierOffer <= 100 &&
                creatorFeeMultiplierOffer <= 100,
            "Fee multiplier to big"
        );
        ...
        emit TierChanged(
            tier,
            tokenBalance,
            marketFeeMultiplierSale,
            creatorFeeMultiplierSale,
            marketFeeMultiplierOffer,
            creatorFeeMultiplierOffer
        );
    }
    ...
    function setWTFuelAddress(address wTFuel) external onlySuperAdmin {
        WTFuel = wTFuel;
    }

    function setOpenThetaTokenAddress(address OTToken) external onlySuperAdmin {
    ...
    function createMarketSale(
        ...
        (
            uint marketFeeMultiplier,
            uint creatorFeeMultiplier
        ) = getFeeMultiplier(sellerAddress, false);
    ...
    function createMarketItemOfferTNT20(
        address nftContract,
        uint256 itemId,
        uint256 offerPrice
    ) public nonReentrant {
    ...
    function acceptMarketItemOfferTNT20(
        ...
        if (
            IERC20(WTFuel).allowance(bidder, address(this)) <
        ...
        (
            uint marketFeeMultiplier,
            uint creatorFeeMultiplier
        ) = getFeeMultiplier(idToMarketItem[itemId].seller, true);
    ...
    function cancelMarketItemOfferTNT20(uint256 itemId) public nonReentrant {
        require(
            ...
            "You have to be the bidder to cancel"
        );
        require(idToMarketItem[itemId].isSold == false, "Item is already sold");
    ...
    function flipTiersState() public onlySuperAdmin {
        ...
        if (tiersAreActive && openThetaToken != address(0x0)) {
            ...
            if (offer) {
                if (userTokenBalance >= sellerTiers[2].tokenBalance) {
                    return (
                        sellerTiers[2].marketFeeMultiplierOffer,
                        sellerTiers[2].creatorFeeMultiplierOffer
                    );
                } else if (userTokenBalance >= sellerTiers[1].tokenBalance) {
                    return (
                        sellerTiers[1].marketFeeMultiplierOffer,
                        sellerTiers[1].creatorFeeMultiplierOffer
                    );
                } else if (userTokenBalance >= sellerTiers[0].tokenBalance) {
                    return (
                        sellerTiers[0].marketFeeMultiplierOffer,
                        sellerTiers[0].creatorFeeMultiplierOffer
                    );
                } else {
                    return (100, 100);
                }
                ...
                if (userTokenBalance >= sellerTiers[2].tokenBalance) {
                    return (
                        sellerTiers[2].marketFeeMultiplierSale,
                        sellerTiers[2].creatorFeeMultiplierSale
                    );
                } else if (userTokenBalance >= sellerTiers[1].tokenBalance) {
                    return (
                        sellerTiers[1].marketFeeMultiplierSale,
                        sellerTiers[1].creatorFeeMultiplierSale
                    );
                } else if (userTokenBalance >= sellerTiers[0].tokenBalance) {
                    return (
                        sellerTiers[0].marketFeeMultiplierSale,
                        sellerTiers[0].creatorFeeMultiplierSale
                    );
                } else {
                    return (100, 100);
                }
